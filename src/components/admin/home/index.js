...
export default class Home extends Component {
    ...
    constructor(props){
        ...
        this.state={
            getList:[], isloaded:"", status:"", statusList:[], offset: 0,
            perPage: 5,
            orgtableData: [],
            currentPage: 0
        }
   ...
   async getstatusList(){
        ...
        const result=await getOrderList.getOrderCount();
        if (result) {
            this.setState({
                statusList:result.data,isloaded:false
            })
        } 
    }
   ...
   async handleChangeStatus(e){
    const{value}=e.target
        this.setState({isloaded:true,status:value})
        const result=await getOrderList.getOrderByStatus(value);
        if (result) {
            this.setState({
                getList:result.order,isloaded:false
            })
        } 
    }
    ...
    async getOrderlist(){
        this.setState({isloaded:true})
        const result=await getOrderList.getAllOrderList();
        const data=result.order;
        
        if (result) {
            const slice=data.slice(this.state.offset,this.state.offset+this.state.perPage)
            this.setState({getList:slice,orgtableData:data,isloaded:false,
                  ...
                  <div className="col-xl-12 col-md-12">
                      ...
                      <div className="card card-static-2 mb-30">
                          ...
                          <div className="card-title-2">
                              <h4>Recent Orders</h4>
                              <button className="view-btn hover-btn" onClick={()=>{this.viewAll()}}>View All</button>
                          </div>
